...
De Lead Web/
├─ apps/
│  ├─ deleadint/       Astro   → deleadint.com                (hub)
│  ├─ walk2lead/       Astro   → w2l.deleadint.com            (alias: walk2lead.deleadint.com → 301)
│  ├─ makerchamps/     Astro   → mc.deleadint.com             (alias: makerchamps.deleadint.com → 301)
│  ├─ corporate/       Astro   → corporate.deleadint.com
│  ├─ dli-education/    Astro   → edu.deleadint.com            (/, /students, /professionals)
...
│  ├─ ui/       shared React components (used by dashboard + Astro islands)  (@delead/ui)
...
|---|---|---|
| Marketing sites | **Astro 5** + **Tailwind v4** (`@tailwindcss/vite`) | static output (`output: 'static'`) |
| Astro interactivity | **React islands** (`@astrojs/react`), hydrated `client:visible` | only where needed (forms, counters, a few hero effects) |
| Marketing animation | CSS transitions + **Intersection Observer** reveal util + **Astro View Transitions**. `motion` (framer-motion) **only inside islands**, sparingly. | **No** Lenis smooth-scroll, **no** GSAP ScrollTrigger pinning — those are the cause of "jittery / shaking / laggy". Everything respects `prefers-reduced-motion`. |
...
| Hub deploys | Cloudflare Pages (5 Astro projects) + Vercel (dashboard, tinkerchamps) | see §8 |
| Content → site refresh | dashboard "Publish" → calls that site's **Cloudflare deploy hook** | sites are static; publish = rebuild |
...
| `tc_bookings` | studentName, studentAge, classGrade, gender, school, district, place, parentName, email, phone, selectedProgram, `created_at`. Read-only list + CSV. |
...
- **Publish model:** content tables have `is_active` / `status`. Saving stores the change; a per-vertical **"Publish to site"** button (top of each vertical section) calls the deploy hook and shows "Site rebuilding… (~1 min)". A subtle "3 unpublished changes" badge tracks drift.
...
**3. TinkerChamps**
   ...
   - *Bookings* — read-only table (Student · Age · Class · Parent · Phone · Program · Date), filters (program, district, date), CSV. Detail sheet shows all fields.
...
Each Astro site fetches its content **at build time** from Postgres (via `@delead/db`, read-only
query in the page frontmatter). No client-side DB calls. Sites rebuild when:
- code is pushed (Cloudflare auto), or
- the dashboard "Publish" button fires that site's Cloudflare **deploy hook**.

Forms: every site ships the shared `<LeadForm>` island → `POST {PUBLIC_LEAD_ENDPOINT}` (the
...
- Astro sites don't call storage — they read `asset_id` rows at build and derive the URL the same way.
...
**Cloudflare Pages** — one project per Astro app:
- Connect `github.com/webdelead/De-Lead-Web`; preset Astro.
- Build cmd `pnpm install && pnpm --filter <app> build`; output `apps/<app>/dist`; root `/`.
- Build watch paths: `apps/<app>/*`, `packages/*`.
- Env: `DATABASE_URL` (read replica / pooler), `PUBLIC_LEAD_ENDPOINT`.
- Custom domain → add subdomain → copy CNAME target → add `CNAME` in Hostinger. SSL auto.
- Create a **Deploy Hook** → paste into dashboard `.env` (`DEPLOY_HOOK_*`).

**Vercel** — two projects, same repo:
- `dashboard`: Root Directory `apps/dashboard`; domain `admin.deleadint.com`; all `.env` vars.
- `tinkerchamps`: Root Directory `apps/tinkerchamps`; domain `tc.deleadint.com`.
...
5. `[ ]` Astro sites, in order: `deleadint` → `walk2lead` → `makerchamps` → `corporate` → `dli-education`. Each: port sections to Tailwind, wire build-time content, `<LeadForm>`, View Transitions, reveal util.
...
- Framework migration of the Astro sites (staying static is deliberate).
